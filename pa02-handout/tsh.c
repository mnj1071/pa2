...
void eval(char *cmdline) 
{
    ...
    pid_t pid;
    sigset_t mask;
    int bg = parseline(cmdline, argv); // return treu if bg, int parseline(const char *cmdline, char **argv)
	...
	if((pid = fork())<0) unix_error("fork error");
	    ...
	    sigprocmask(SIG_BLOCK, &mask, NULL);
	    if(execve(argv[0], argv, environ)<0) exit(0);
	}

    k = addjob(jobs, pid, bg+1, cmdline);	
    if(!bg) { 
	if(k) 
        {
